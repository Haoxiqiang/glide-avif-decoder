...
public class AvifSequenceDrawable extends Drawable implements Animatable {
    ...
    private static final String TAG = "AvifSequenceDrawable";
    public static final boolean DEBUG = true;
    /**
     * These constants are chosen to imitate common browser behavior for WebP/GIF. If other decoders
     * are added, this behavior should be moved into the WebP/GIF decoders.
     *
     * <p>Note that 0 delay is undefined behavior in the GIF standard.
     */
    ...
    private static final BitmapProvider sAllocatingBitmapProvider =
            new BitmapProvider() {
                @Override
     ...
     *
     * <p>Must be one of LOOP_INF, LOOP_DEFAULT, or LOOP_FINITE.
     */
                ...
                public void run() {
                    synchronized (mLock) {
                        ...
                        try {
                            ...
                            //                    Log.d(TAG, "run: invalidateMs " + invalidateTimeMs
                            // + "   " + mAvifDecoder.getImageIndex());
                        } catch (Exception e) {
                        ...
                        mNextFrameRenderTime =
                                exceptionDuringDecode ? Long.MIN_VALUE : invalidateTimeMs + start;
    ...
    private static Bitmap acquireAndValidateBitmap(
            BitmapProvider bitmapProvider, int minWidth, int minHeight) {
     ...
     *
     * <p>If no BitmapProvider is attached to the drawable, recycle() is called on the Bitmaps.
     */
    public void destroy() {
    ...
    /**
     * Default executor for rendering tasks - {@link
     * java.util.concurrent.ScheduledThreadPoolExecutor} with 1 worker thread and {@link
    ...
    public interface OnFinishedListener {
         ...
         *
         * <p>Note that this is will not be called if the drawable is explicitly stopped, or marked
         * invisible.
         */
    ...
    public interface BitmapProvider {
        ...
        /**
         * Called by FrameSequenceDrawable to release a Bitmap it no longer needs. The Bitmap will
         * no longer be used at all by the drawable, so it is safe to reuse elsewhere.
         *
         * <p>This method may be called by FrameSequenceDrawable on any thread.
         */
        public void releaseBitmap(Bitmap bitmap);
    }
}
